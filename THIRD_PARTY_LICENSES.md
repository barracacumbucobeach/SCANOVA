...
| WiX Toolset | 4.0.6 | MS-RL (Microsoft Reciprocal License — verificada no `PackageLicenseExpression` do pacote `wix` no NuGet). Ferramenta de **build**, nunca distribuída com o aplicativo nem vinculada a ele (o instalador `.msi` que ela produz não contém nenhum código do WiX) — mesmo raciocínio já aplicado a `Microsoft.Windows.SDK.BuildTools` acima: um compilador não "contamina" a licença do que ele compila | Gera o instalador tradicional (MSI) do SCANOVA (Fase 11, seção 91/118) | https://github.com/wixtoolset/wix |
